...
describe('platformPlans.routes', () => {
    it('protège et valide la liste des plans avant le controller', async () => {
        ...
        expect(handlers.listPlans).toHaveBeenCalledOnce();
        expect(handlers.createPlan).not.toHaveBeenCalled();
    });
    ...
    it('protège et valide la création d’un plan avant le controller', async () => {
        ...
        expect(permissionMiddleware).toHaveBeenCalledOnce();
        expect(validationMiddleware).toHaveBeenCalledOnce();
        expect(handlers.createPlan).toHaveBeenCalledOnce();
    });
    ...
    it('protège et valide la mise à jour d’un plan avant le controller', async () => {
        ...
        expect(permissionMiddleware).toHaveBeenCalledOnce();
        expect(validationMiddleware).toHaveBeenCalledOnce();
        expect(handlers.updatePlan).toHaveBeenCalledOnce();
    });
    ...
    it('protège et valide l’archivage d’un plan avant le controller', async () => {
        ...
        const response = await request(app)
            .patch(`/platform/plans/${planId}/archive`);
        ...
        expect(permissionMiddleware).toHaveBeenCalledOnce();
        expect(validationMiddleware).toHaveBeenCalledOnce();
        expect(handlers.archivePlan).toHaveBeenCalledOnce();
    });
});