/**
 * Résultat fonctionnel de l'action auditée.
 *
 * Le statut décrit l'issue de l'action métier, pas la réussite technique de
 * l'écriture du journal d'audit.
 */
const AUDIT_STATUS = Object.freeze({
    SUCCESS: 'success',
    FAILED: 'failed',
});

/**
 * Types de ressources pouvant être visées par une action auditée.
 */
const AUDIT_ENTITY_TYPE = Object.freeze({
    AUTH_SESSION: 'AuthSession',
    ENTITLEMENT_OVERRIDE: 'EntitlementOverride',
    FILE: 'File',
    ORGANIZATION: 'Organization',
    PLAN: 'Plan',
    PLATFORM_INVITATION: 'PlatformInvitation',
    PLATFORM_ROLE: 'PlatformRole',
    PLATFORM_TEAM_MEMBER: 'PlatformTeamMember',
    ROLE: 'Role',
    SUBSCRIPTION: 'Subscription',
    USER: 'User',
    WORKSPACE: 'Workspace',
    WORKSPACE_INVITATION: 'WorkspaceInvitation',
    WORKSPACE_MEMBER: 'WorkspaceMember',
});

/**
 * Vocabulaire stable des actions sensibles de la plateforme.
 */
const AUDIT_ACTION = Object.freeze({
    LOGIN_SUCCESS: 'LOGIN_SUCCESS',
    LOGIN_FAILED: 'LOGIN_FAILED',
    LOGOUT: 'LOGOUT',
    LOGOUT_ALL: 'LOGOUT_ALL',
    PASSWORD_CHANGED: 'PASSWORD_CHANGED',
    PASSWORD_RESET_COMPLETED: 'PASSWORD_RESET_COMPLETED',

    USER_CREATED: 'USER_CREATED',
    USER_PROFILE_UPDATED: 'USER_PROFILE_UPDATED',
    USER_DISABLED: 'USER_DISABLED',
    USER_ENABLED: 'USER_ENABLED',
    USER_DELETION_REQUESTED: 'USER_DELETION_REQUESTED',
    USER_CLOSED: 'USER_CLOSED',
    USER_PLATFORM_ROLE_UPDATED: 'USER_PLATFORM_ROLE_UPDATED',

    SESSION_REVOKED: 'SESSION_REVOKED',
    SESSION_REUSE_DETECTED: 'SESSION_REUSE_DETECTED',

    WORKSPACE_CREATED: 'WORKSPACE_CREATED',
    WORKSPACE_UPDATED: 'WORKSPACE_UPDATED',
    WORKSPACE_SUSPENDED: 'WORKSPACE_SUSPENDED',
    WORKSPACE_REACTIVATED: 'WORKSPACE_REACTIVATED',
    WORKSPACE_ARCHIVED: 'WORKSPACE_ARCHIVED',
    WORKSPACE_CLOSED: 'WORKSPACE_CLOSED',
    WORKSPACE_OWNERSHIP_TRANSFERRED: 'WORKSPACE_OWNERSHIP_TRANSFERRED',

    MEMBER_INVITED: 'MEMBER_INVITED',
    MEMBER_INVITATION_ACCEPTED: 'MEMBER_INVITATION_ACCEPTED',
    MEMBER_INVITATION_REVOKED: 'MEMBER_INVITATION_REVOKED',
    MEMBER_INVITATION_RESENT: 'MEMBER_INVITATION_RESENT',
    MEMBER_ROLE_UPDATED: 'MEMBER_ROLE_UPDATED',
    MEMBER_SUSPENDED: 'MEMBER_SUSPENDED',
    MEMBER_REMOVED: 'MEMBER_REMOVED',

    PLATFORM_INVITATION_CREATED: 'PLATFORM_INVITATION_CREATED',
    PLATFORM_INVITATION_ACCEPTED: 'PLATFORM_INVITATION_ACCEPTED',
    PLATFORM_INVITATION_REVOKED: 'PLATFORM_INVITATION_REVOKED',
    PLATFORM_INVITATION_RESENT: 'PLATFORM_INVITATION_RESENT',
    PLATFORM_MEMBER_ROLE_UPDATED: 'PLATFORM_MEMBER_ROLE_UPDATED',
    PLATFORM_MEMBER_SUSPENDED: 'PLATFORM_MEMBER_SUSPENDED',
    PLATFORM_MEMBER_REACTIVATED: 'PLATFORM_MEMBER_REACTIVATED',
    PLATFORM_MEMBER_REVOKED: 'PLATFORM_MEMBER_REVOKED',
    PLATFORM_ROLE_CREATED: 'PLATFORM_ROLE_CREATED',
    PLATFORM_ROLE_UPDATED: 'PLATFORM_ROLE_UPDATED',
    PLATFORM_ROLE_ARCHIVED: 'PLATFORM_ROLE_ARCHIVED',

    ROLE_CREATED: 'ROLE_CREATED',
    ROLE_UPDATED: 'ROLE_UPDATED',
    ROLE_DELETED: 'ROLE_DELETED',

    PLAN_CREATED: 'PLAN_CREATED',
    PLAN_UPDATED: 'PLAN_UPDATED',
    PLAN_ARCHIVED: 'PLAN_ARCHIVED',

    SUBSCRIPTION_CREATED: 'SUBSCRIPTION_CREATED',
    SUBSCRIPTION_UPDATED: 'SUBSCRIPTION_UPDATED',
    SUBSCRIPTION_CANCELLATION_SCHEDULED:
        'SUBSCRIPTION_CANCELLATION_SCHEDULED',
    SUBSCRIPTION_CANCELED: 'SUBSCRIPTION_CANCELED',
    SUBSCRIPTION_EXPIRED: 'SUBSCRIPTION_EXPIRED',
    SUBSCRIPTION_ACTIVATED_FROM_TRIAL:
        'SUBSCRIPTION_ACTIVATED_FROM_TRIAL',
    SUBSCRIPTION_PROMOTION_APPLIED:
        'SUBSCRIPTION_PROMOTION_APPLIED',
    SUBSCRIPTION_RESUMED: 'SUBSCRIPTION_RESUMED',
    SUBSCRIPTION_DOWNGRADE_SCHEDULED:
        'SUBSCRIPTION_DOWNGRADE_SCHEDULED',
    SUBSCRIPTION_DOWNGRADE_REVOKED:
        'SUBSCRIPTION_DOWNGRADE_REVOKED',
    SUBSCRIPTION_DOWNGRADE_APPLIED:
        'SUBSCRIPTION_DOWNGRADE_APPLIED',

    ENTITLEMENT_OVERRIDE_CREATED:
        'ENTITLEMENT_OVERRIDE_CREATED',
    ENTITLEMENT_OVERRIDE_UPDATED:
        'ENTITLEMENT_OVERRIDE_UPDATED',
    ENTITLEMENT_OVERRIDE_REVOKED:
        'ENTITLEMENT_OVERRIDE_REVOKED',

    FILE_UPLOADED: 'FILE_UPLOADED',
    FILE_UPLOAD_REJECTED: 'FILE_UPLOAD_REJECTED',
    FILE_DELETED: 'FILE_DELETED',
    FILE_PURGED: 'FILE_PURGED',

    ORGANIZATION_CREATED: 'ORGANIZATION_CREATED',
    ORGANIZATION_UPDATED: 'ORGANIZATION_UPDATED',
    ORGANIZATION_SUSPENDED: 'ORGANIZATION_SUSPENDED',
    ORGANIZATION_REACTIVATED: 'ORGANIZATION_REACTIVATED',
});

export {
    AUDIT_ACTION,
    AUDIT_ENTITY_TYPE,
    AUDIT_STATUS,
};
